  ...
  .handler(async ({ data, context }): Promise<AcceptOfferResult> => {
    ...
    if (trainer.current_team_id) {
      ...
      await supabase
        ...
        .eq("team_id", (trainer.current_team_id as any) || (null as any))
        .eq("trainer_id", trainer.id);
    }
    ...
    const { data: newClubBuildings } = await supabase
      .from("buildings")
      .select("id")
      .eq("team_id", (newTeam.id as any) || (null as any));
    if (newClubBuildings && newClubBuildings.length > 0) {
      ...
      const { error: createBuildingsError } = await supabase.from("buildings").insert([
        { trainer_id: trainer.id, team_id: (newTeam.id as any) || (null as any), building_type: "estadio", level: baseline.estadio },
        { trainer_id: trainer.id, team_id: (newTeam.id as any) || (null as any), building_type: "ct_treino", level: baseline.ct_treino },
        { trainer_id: trainer.id, team_id: (newTeam.id as any) || (null as any), building_type: "centro_medico", level: baseline.centro_medico },
      ]);
