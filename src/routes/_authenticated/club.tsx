...
function MonthlyClubPage() {
  ...
  return <GamePageShell title="Clube Mensal" subtitle="Assiduidade também vale benefícios" gems={data.gems}>
      ...
      <Card><CardHeader><CardTitle className="flex items-center gap-2 text-base"><CalendarDays className="h-4 w-4" />Missões semanais</CardTitle></CardHeader><CardContent className="space-y-3">{data.tasks.map((task: any) => <div key={task.key} className="flex flex-col gap-3 rounded-md border p-3 sm:flex-row sm:items-center"><div className="min-w-0 flex-1"><div className="flex justify-between gap-2 text-sm"><span className="font-medium">{task.label}</span><span>{task.current}/{task.target}</span></div><Progress className="mt-2 h-1.5" value={(task.current / task.target) * 100} /></div><Button size="sm" className="w-full sm:w-auto" disabled={!task.complete || task.claimed || claimMutation.isPending} onClick={() => claimMutation.mutate(task.key)}>{task.claimed ? "Resgatado" : `+${task.reward} 💎`}</Button></div>)}</CardContent></Card>
