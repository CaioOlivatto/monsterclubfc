...
DECLARE
  ...
  v_reset_energy boolean := COALESCE((payload->>'reset_energy')::boolean, false);
  v_reset_rest boolean := COALESCE((payload->>'reset_rest_pool')::boolean, COALESCE((payload->>'reset_energy')::boolean, false));
  ...
  v_energy_reset integer := 0;
  v_rest_reset integer := 0;
BEGIN
  ...
  -- Pré-temporada: energia total para todas as criaturas do mundo do treinador.
  -- Não altera moral nem status de lesão.
  IF v_reset_energy THEN
    UPDATE public.creatures c
       SET energy = 100,
           rest_completes_at = NULL,
           updated_at = now()
     WHERE c.retired = false
       AND (
         c.owner_trainer_id = v_trainer_id
         OR EXISTS (
           SELECT 1
             FROM public.teams tm
             JOIN public.competitions comp ON comp.id = tm.competition_id
            WHERE tm.id = c.owner_team_id
              AND comp.trainer_id = v_trainer_id
         )
       );
    GET DIAGNOSTICS v_energy_reset = ROW_COUNT;
  END IF;

  -- Pré-temporada: pool de "Descansar" volta a 3/3 grátis e zera usos pagos.
  IF v_reset_rest THEN
    UPDATE public.trainers
       SET rest_free_charges = 3,
           rest_paid_uses = 0,
           rest_pool_zeroed_at = NULL,
           updated_at = now()
     WHERE id = v_trainer_id;
    GET DIAGNOSTICS v_rest_reset = ROW_COUNT;
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'energy_reset', v_energy_reset,
    'rest_pool_reset', v_rest_reset
  );
END;
$function$;